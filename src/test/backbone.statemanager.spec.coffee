...
StateManager = require '../backbone.statemanager'

describe 'Backbone.StateManager', ->
  beforeEach -> @_states = helper().states

  afterEach -> delete @_states

  it 'exists under Backbone.StateManager', ->
    expect(Backbone.StateManager).toBeDefined()
  ...
  describe 'constructor', ->
    ...
    it 'calls addState with passed state', ->
      ...
      expect(stateManager.states.add)
        .toHaveBeenCalledWith 'noTransitions', jasmine.any Object
  ...
  describe 'prototype', ->
    ...
    describe 'initialize', ->
      it 'calls triggerState on the first state found that has initial
      : true set on it', ->
        spyOn Backbone.StateManager.prototype, 'triggerState'

        @stateManager.initialize()
        expect(@stateManager.triggerState)
          .toHaveBeenCalledWith 'withInitial', jasmine.any Object
    ...
    describe 'addState', ->
      ...
      it 'sets the state passed to states with the states callback', ->
        ...
        expect(@stateManager.states.states.noTransitions)
      ...
      it 'triggers remove:state and passes state name', ->
        ...
        expect(@stateManager.trigger)
          .toHaveBeenCalledWith 'add:state', 'noTransitions'
    ...
    describe 'removeState', ->
      ...
      it 'triggers remove:state and passes state name', ->
        ...
        @stateManager.removeState 'noTransitions'
        expect(@stateManager.trigger)
          .toHaveBeenCalledWith 'remove:state', 'noTransitions'
    ...
    describe 'triggerState', ->
      ...
      with new state name', ->
        ...
        expect(@stateManager.exitState)
          .toHaveBeenCalledWith toState : 'foo', fromState : 'bar'

      it 'calls enterState for the new state', ->
        @stateManager.triggerState 'foo'
        expect(@stateManager.enterState)
      ...
      describe 'the current state is the same as the new state', ->
        ...
        it 're-enters the state if optoins.reEnter is set', ->
          expect(@stateManager.triggerState 'foo', reEnter : true)
            .not.toEqual false
          expect(@stateManager.exitState)
    ...
    describe 'exitState', ->
      ...
      describe 'with invalid parameters', ->
        ...
        it 'returns false if the states exit property is not a method', ->
          @stateManager.states.find.and.returnValue @_states.nonMethodExit
          expect(@stateManager.exitState()).toBeFalsy()

        it 'returns false if the currentState does not exist', ->
          @stateManager.states.find.and.returnValue false
          expect(@stateManager.exitState()).toBeFalsy()
      ...
      describe 'with valid parameters', ->
        ...
        beforeEach ->
          ...
          @stateManager.states.find.and.returnValue(
            new Backbone.StateManager.State(
              'noTransitions', @_states.noTransitions
            )
          )
        ...
        it 'calls the exit method on the state', ->
          expect(@_states.noTransitions.exit)
            .toHaveBeenCalledWith jasmine.any Object
        ...
        it 'deletes the currentState', ->
          expect(@stateManager.currentState).toBeUndefined()

      describe 'on states with transitions set', ->
        beforeEach ->
          @stateManager.states.find.and.returnValue(
            exit : (->), __proto__ : Backbone.StateManager.State.prototype
          )
          @transitionCallback = jasmine.createSpy 'transitionCallback'
          spyOn(Backbone.StateManager.State.prototype, 'findTransition')
            .and.returnValue @transitionCallback
    ...
    describe 'enterState', ->
      ...
      describe 'with invalid parameters', ->
        ...
        it 'returns false if the states enter property is not a method', ->
          @stateManager.states.find.and.returnValue @_states.nonMethodEnter
      ...
      describe 'with valid parameters', ->
        ...
        it 'calls the enter method on the state', ->
          expect(@_states.noTransitions.enter)
            .toHaveBeenCalledWith jasmine.any Object
        ...
        it 'sets the currentState', ->
          expect(@stateManager.currentState).toEqual 'noTransitions'

      describe 'on states with transitions set', ->
        beforeEach ->
          @stateManager.states.find.and.returnValue(
            enter : (->), __proto__ : Backbone.StateManager.State.prototype
          )
          ...
          @stateManager.enterState(
            'enterTransition', fromState : 'exitTransition'
          )
  ...
  describe 'addStateManager', ->
    it 'creates a new StateManager', ->
      ...
      spy = spyOn(Backbone, 'StateManager').and.callThrough()
    ...
    it 'if states is set to a function, executes it', ->
      StateManager = Backbone.StateManager
      states = jasmine.createSpy('states').and.returnValue @_states
    ...
    it 'allows callthrough on the target for triggerState', ->
      ...
      expect(Backbone.StateManager.prototype.triggerState)
    ...
    it 'does not call initialize if options.initialize is set to false(y)', ->
      ...
      _.each [false, null, 0], (value) ->
        ...
        expect(Backbone.StateManager.prototype.initialize)
