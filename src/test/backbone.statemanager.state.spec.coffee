    ...
    describe 'findTransition', ->
      describe 'straight match', ->
        it 'finds functions who have a key matching the type and name', ->
          test = new Backbone.StateManager.State(
            'foo', { transitions : 'onFoo:Bar*splat' : -> }
          )
          expect(test.findTransition 'onFoo', 'Bar123')
      ...
      describe 'not match', ->
        it 'finds functions who have a key that does not match the name', ->
          ...
          test = new Backbone.StateManager.State(
            'foo', { transitions : transitions }
          )
          expect(test.findTransition('onFoo', 'Baz123')())
            .toEqual 'not bar match'
